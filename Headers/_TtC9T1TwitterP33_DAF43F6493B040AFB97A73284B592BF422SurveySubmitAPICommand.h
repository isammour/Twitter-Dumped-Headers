//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <TFSTwitterAPICommand/TFSTwitterAPICommand.h>

__attribute__((visibility("hidden")))
@interface _TtC9T1TwitterP33_DAF43F6493B040AFB97A73284B592BF422SurveySubmitAPICommand : TFSTwitterAPICommand
{
    // Error parsing type: , name: surveyId
    // Error parsing type: , name: surveyVersion
    // Error parsing type: , name: surveyResponses
}

- (CDUnknownBlockType).cxx_destruct;
- (id)initWithService:(id)arg1 accountID:(id)arg2 authContext:(id)arg3 networkTransactionMetrics:(id)arg4 source:(unsigned long long)arg5 completionBlock:(CDUnknownBlockType)arg6;
- (id)request;

@end

