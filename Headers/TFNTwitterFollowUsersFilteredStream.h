//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <T1Twitter/TFNTwitterUsersStream.h>

@class TFNTwitterUserDataSource;

@interface TFNTwitterFollowUsersFilteredStream : TFNTwitterUsersStream
{
    TFNTwitterUserDataSource *_userDataSource;
}

@property(retain, nonatomic) TFNTwitterUserDataSource *userDataSource; // @synthesize userDataSource=_userDataSource;
- (void).cxx_destruct;
- (void)_t1_userBlockingDidUpdate:(id)arg1;
- (_Bool)shouldShowViewerInFollowListForUserDataSource:(id)arg1;
- (_Bool)shouldShowUserInViewerFollowList:(id)arg1;
- (_Bool)isEmpty;
- (id)streamObjects;
- (id)init;

@end

