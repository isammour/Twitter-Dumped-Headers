//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <T1Twitter/T1UsersItemsTableViewController.h>

#import <T1Twitter/TFNModalSheetViewControllerDelegate-Protocol.h>

@class NSString, TFSTwitterScribeContext, UIBarButtonItem;
@protocol T1ComposeReplyUsersTableViewControllerDelegate;

@interface T1ComposeReplyUsersTableViewController : T1UsersItemsTableViewController <TFNModalSheetViewControllerDelegate>
{
    TFSTwitterScribeContext *_scribeContext;
    _Bool _editable;
    _Bool _bulkUpdateInProgress;
    NSString *_topOfStreamMessage;
    id <T1ComposeReplyUsersTableViewControllerDelegate> _replyUsersDelegate;
    UIBarButtonItem *_doneBarButtonItem;
}

@property(retain, nonatomic) UIBarButtonItem *doneBarButtonItem; // @synthesize doneBarButtonItem=_doneBarButtonItem;
@property(nonatomic) _Bool bulkUpdateInProgress; // @synthesize bulkUpdateInProgress=_bulkUpdateInProgress;
@property(nonatomic) __weak id <T1ComposeReplyUsersTableViewControllerDelegate> replyUsersDelegate; // @synthesize replyUsersDelegate=_replyUsersDelegate;
@property(copy, nonatomic) NSString *topOfStreamMessage; // @synthesize topOfStreamMessage=_topOfStreamMessage;
@property(nonatomic, getter=isEditable) _Bool editable; // @synthesize editable=_editable;
- (void).cxx_destruct;
- (void)private_didTapDone:(id)arg1;
- (id)private_otherPeopleInConversationItemWithToggle:(_Bool)arg1;
- (id)private_gapItem;
- (unsigned long long)private_excludableUsersCount;
- (unsigned long long)private_excludedUsersCount;
- (id)private_realUsers;
- (void)private_unselectAllButtonPressed:(id)arg1;
- (void)private_selectAllButtonPressed:(id)arg1;
- (void)private_toggleAllButtonPressed:(unsigned long long)arg1;
- (void)private_logImpression;
- (id)scribe;
- (id)scribeContext;
- (void)modalSheetViewController:(id)arg1 willTransitionToLayoutMode:(long long)arg2 withTransitionCoordinator:(id)arg3;
- (_Bool)userRowAdapter:(id)arg1 isUserCheckmarkInteractionEnabled:(id)arg2;
- (_Bool)userRowAdapter:(id)arg1 isUserChecked:(id)arg2;
- (void)private_updateSelectionForUser:(id)arg1 checked:(_Bool)arg2;
- (void)userRowAdapter:(id)arg1 userCheckmarkDidChangeValue:(id)arg2 checked:(_Bool)arg3;
- (void)setUserFollowControlType:(unsigned long long)arg1;
- (void)didSelectItem:(id)arg1 atIndexPath:(id)arg2;
- (id)previewingViewControllerForItem:(id)arg1 location:(struct CGPoint)arg2 inCell:(id)arg3 atIndexPath:(id)arg4 outSourceRect:(out struct CGRect *)arg5;
- (void)viewDidLoad;
- (id)usersSection;
- (void)update:(_Bool)arg1;
- (id)initWithAccount:(id)arg1 userIDs:(id)arg2 scribeContext:(id)arg3;
- (id)initWithAccount:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

