//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <T1Twitter/T1UsersItemsTableViewController.h>

@class T1AppNavigationContext, T1FollowerRequests, T1PushLandingHelper, T1PushLandingItem, T1ViewControllerDidPresentQueue;

@interface T1FollowerRequestsViewController : T1UsersItemsTableViewController
{
    _Bool _shouldShowLandingItem;
    _Bool _needsFollowerRequestUpdate;
    T1AppNavigationContext *_navigationContext;
    T1PushLandingHelper *_pushLandingHelper;
    T1PushLandingItem *_landingItem;
    T1FollowerRequests *_followerRequests;
    T1ViewControllerDidPresentQueue *_didPresentQueue;
}

@property(retain, nonatomic) T1ViewControllerDidPresentQueue *didPresentQueue; // @synthesize didPresentQueue=_didPresentQueue;
@property(retain, nonatomic) T1FollowerRequests *followerRequests; // @synthesize followerRequests=_followerRequests;
@property(nonatomic) _Bool needsFollowerRequestUpdate; // @synthesize needsFollowerRequestUpdate=_needsFollowerRequestUpdate;
@property(nonatomic) _Bool shouldShowLandingItem; // @synthesize shouldShowLandingItem=_shouldShowLandingItem;
@property(retain, nonatomic) T1PushLandingItem *landingItem; // @synthesize landingItem=_landingItem;
@property(retain, nonatomic) T1PushLandingHelper *pushLandingHelper; // @synthesize pushLandingHelper=_pushLandingHelper;
@property(retain, nonatomic) T1AppNavigationContext *navigationContext; // @synthesize navigationContext=_navigationContext;
- (void).cxx_destruct;
- (id)scribeImpressionParameters;
- (id)scribeSection;
- (id)scribePage;
- (id)scribe;
- (long long)dashContentItemType;
- (void)willDisplayCell:(id)arg1 forItem:(id)arg2 atIndexPath:(id)arg3;
- (void)_streamDidUpdate:(id)arg1;
- (id)_emptyStreamItem;
- (void)_t1_updateUsers;
- (void)_t1_updateFollowerRequests;
- (void)update:(_Bool)arg1;
- (id)title;
- (void)viewDidAppear:(_Bool)arg1;
- (id)initWithAccount:(id)arg1;

@end

